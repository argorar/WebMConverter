...
{
    public partial class ConverterFFmepeg : Form
    {
        ...
        public ConverterFFmepeg(string input, string[] args, string output)
        {
            InitializeComponent();
        ...
        void AddInputFileToArguments(ref string argument, int argumentNumber)
        {
            if (argumentNumber > 1 || argument.Contains("vidstab"))
                return;
            if (_needToPipe && argumentNumber > 1)
                argument = $@"-f nut -i pipe:0 {argument}";
            else
                argument = $@"-i ""{_infile}"" {argument}";
        }
